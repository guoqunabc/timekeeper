...
class Timer {
    ...
    updateOvertimeAnimation() {
        ...
        if (overtimeMinutes >= 5) {
            ...
            this.timerElement.style.setProperty('--blink-duration', `${duration.toFixed(2)}s`);
        }
    }
    ...
    updateDisplay() {
        ...
        // 只更新数字部分，保持结构不变
        this.timerElement.querySelector('.timer-minutes').textContent = 
            minutes.toString().padStart(2, '0');
        this.timerElement.querySelector('.timer-seconds').textContent = 
    ...
    reset() {
        ...
        // 重置样式
        this.timerElement.style.color = 'white';
        this.timerElement.classList.remove('overtime-animation', 'warning-animation');
    ...
    updateRecordList() {
        this.recordList.innerHTML = this.records.map((record, index) => {
            ...
            const overtimeText = record.overtimeSeconds > 0 
            ...
            return `
                <div class="record-item">
                    ...
                    <button class="delete-record-btn" onclick="timer.deleteRecord(${index})" title="删除记录">×</button>
                </div>
            `;
        }).join('');
    }
            ...
            const csvContent = [
                ...
                ...this.records.map(record => {
                    ...
                    return [
                        ...
                        record.overtimeSeconds > 0 ? `${overtimeMin}:${overtimeSec.toString().padStart(2, '0')}` : '0:00',
                        record.timestamp
                    ];
                })
